...
const makeDirectConversationId = (uid1, uid2) => {
  return `${DIRECT_CONVERSATION_PREFIX}${[uid1, uid2]
    .sort()
    .join('_')}`;
};
...
const containsMention = (text, username) => {
...
export default function ChatScreen({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error('Chat profile listener error:', error);
      }
    );
   ...
   * Real-time listener.
   *
   * No fake chat objects are inserted.
   */
  ...
  const isFriend = async (targetUid) => {
  ...
  const sendFriendRequest = async (targetUser) => {
    ...
    if (targetUser.id === currentUser.uid) return;

    try {
      const alreadyFriend =
        await isFriend(targetUser.id);
  ...
  const acceptFriendRequest = async (request) => {
    if (!currentUser || !request?.senderUid) {
      return;
    }
  ...
  const sendDMRequest = async (targetUser) => {
    if (!currentUser || !targetUser) return;
  ...
  const openConversation = async (
    ...
    try {
      await updateDoc(
        ...
        {
          [`urgentFor.${currentUser.uid}`]: false,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      /*
       * Determine who should receive an urgent
       * notification based on @username mentions.
       */
  ...
  const getChatType = (conversation) => {
  ...
  const filteredConversations = useMemo(() => {
    ...
    /*
     * Search locally through currently loaded chats.
     */
    ...
    /*
     * Recent:
     * show conversations that actually have
     * recent message activity, newest first.
     */
     ...
     * Normal third
     *
     * Within the same priority, newest activity
     * appears first.
     */
    ...
    result.sort((a, b) => {
      ...
      const typeA = getChatType(a);
      const typeB = getChatType(b);
  ...
  const toggleGroupFriend = (friendUid) => {
  ...
  const getEmptyMessage = () => {
    ...
    return 'Welcome to Shinzi Chats!\nYou can chat with your friends.';
  };
